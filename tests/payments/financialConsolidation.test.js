...
test('detalle de obligaciones devuelve una fila por concepto con pagos y recibos agregados', async () => {
  ...
  sequelize.query=async(sql,{replacements})=>{sqlSeen=sql;replacementsSeen=replacements;return [
    ...
    {concepto_id:2,fecha:'2026-08-10',hora:'16:25:00',paciente_id:7,paciente:'ANA PEREZ',documento:'123',historia_id:4,sesion_id:12,profesional:'FT A',monto_esperado:200,monto_pagado:0,saldo_pendiente:200,metodos_pago:[],recibos:[],estado_interno:'Pendiente',exonerado:false,activo:true,fecha_ultimo_pago:null},
  ...
  try{
    ...
    assert.equal(result.detalle[0].estadoReporte,'CANCELADO');assert.equal(result.detalle[1].estadoReporte,'NO CANCELADO');assert.equal(result.detalle[1].hora,'16:25:00');assert.equal(result.detalle[2].estadoReporte,'PENDIENTE');
    ...
    assert.match(sqlSeen,/FROM conceptos_cobro c/);assert.match(sqlSeen,/TO_CHAR\(s\.created_at AT TIME ZONE 'America\/La_Paz','HH24:MI:SS'\)[\s\S]*AS hora/);assert.match(sqlSeen,/LEFT JOIN LATERAL/);assert.match(sqlSeen,/SUM\(m\.monto\)/);assert.match(sqlSeen,/ARRAY_AGG\(DISTINCT/);
  }finally{sequelize.query=original}
});
