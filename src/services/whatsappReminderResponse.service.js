...
const processReminderResponse = async ({ conversation, message, reminderModel = WhatsappAppointmentReminder, appointmentModel = Cita, referralModel, transaction, activity, now }) => {
  ...
  if (conversation.paso_actual === CONVERSATION_STEPS.WAITING_NONATTENDANCE_CONFIRMATION) {
    if (choice === '1') { await appointment.update({ estado: 'Cancelada', historial_programacion: [...(Array.isArray(appointment.historial_programacion) ? appointment.historial_programacion : []), { accion: 'CANCELACION_DESDE_RECORDATORIO', estado_anterior: appointment.estado, estado_nuevo: 'Cancelada', registrado_en: now.toISOString() }] }, { transaction }); await cleanupTemporaryWhatsappNoShow(appointment, { transaction }); await markResponse(reminder, 'CANCELAR_CITA', now, transaction); await conversation.update({ paso_actual: CONVERSATION_STEPS.APPOINTMENT_CANCELLED, contexto: {}, ...activity }, { transaction }); return { responseText: `Tu cita fue cancelada correctamente.\n\nEsperamos poder atenderte en otra ocasión 😊\n\nEscribe MENÚ para volver.`, responseKind: 'REMINDER_APPOINTMENT_CANCELLED', conversationStep: CONVERSATION_STEPS.APPOINTMENT_CANCELLED }; }
