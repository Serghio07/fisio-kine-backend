...
const list = async ({ query, model = WhatsappReceptionReferral }) => {
  ...
  else filters.estado = { [Op.in]: ['PENDIENTE', 'EN_ATENCION'] };
...
const mutate = async ({ id, user, action, value, db, model = WhatsappReceptionReferral, now = new Date(), syncNotifications = markReferralNotificationsRead }) => db.transaction(async (transaction) => {
  ...
  changes.historial = history(item, action, user.id, now, action === 'PRIORIDAD' ? { prioridad: value } : {}); await item.update(changes, { transaction }); if (['TOMADA', 'RESUELTA', 'CERRADA'].includes(action)) await syncNotifications({ referralId: item.id, transaction, now }); await audit({ userId: user.id, referral: item, action, transaction }); if (action === 'TOMADA' && typeof transaction.afterCommit === 'function') transaction.afterCommit(() => { const { assignedReferral } = require('./whatsappNotificationTrigger.service'); return assignedReferral(item); }); return item;
});
