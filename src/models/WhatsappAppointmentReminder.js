...
const REMINDER_STATES = Object.freeze(['PENDIENTE', 'PROCESANDO', 'ACEPTADO', 'ENVIADO', 'ENTREGADO', 'LEIDO', 'REINTENTO', 'FALLIDO', 'CANCELADO', 'RESPONDIDO', 'EXPIRADO', 'SIN_DESTINATARIO']);
...
const WhatsappAppointmentReminder = sequelize.define('WhatsappAppointmentReminder', {
  ...
  contacto_id: { type: DataTypes.INTEGER, allowNull: true },
  telefono_normalizado: { type: DataTypes.STRING(15), allowNull: true },
  telefono_fuente: { type: DataTypes.STRING(20), allowNull: false, validate: { isIn: [['PACIENTE', 'CONTACTO']] } },
  parentesco_snapshot: DataTypes.STRING(100), destinatario_nombre_snapshot: DataTypes.STRING(255),
  tipo_recordatorio: { type: DataTypes.STRING(30), allowNull: false, defaultValue: 'CITA_PROXIMA' },
  ...
  validate: {
    destinationMatchesState() {
      if (this.estado === 'SIN_DESTINATARIO' && this.telefono_normalizado) throw new Error('SIN_DESTINATARIO no puede tener teléfono.');
      if (this.estado !== 'SIN_DESTINATARIO' && !this.telefono_normalizado) throw new Error('Un recordatorio enviable requiere teléfono.');
    }
  }
});
